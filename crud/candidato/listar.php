...
<table class="table table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from usuarios where (nivel = 'Cliente' AND nivel = 'Admin') = 'Cliente' order by nome asc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from usuarios where (nivel = 'Cliente' AND nivel = 'Admin') = 'Cliente' and (nome LIKE '$txtbuscar' or cpf LIKE '$txtbuscar') order by nome asc");
